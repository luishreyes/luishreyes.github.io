...
## Generar gráficas científicas con matplotlib (NO dibujar SVG inline a mano)

Cuando una presentación se compone principalmente de **figuras técnicas** (gráficas con ejes, datos, anotaciones, antes/después de diseños) en lugar de fotos de un PDF, **NO** dibujar los SVGs a mano dentro del HTML. Generarlas con matplotlib desde un script Python y exportar PNGs. Razones probadas en `clase-18-comunicacion-visual.html`:

- Tipografía consistente, anti-aliasing real, control sobre espaciado de ejes y posiciones de etiquetas.
- Editable: cambiar un coeficiente y regenerar es trivial; mover una anotación dentro de un SVG inline a mano es fricción pura.
- El propio script sirve como fuente de verdad y queda en `temp/` (gitignored) para regenerar cuando haga falta.

### Workflow

1. **Crear venv aislado** (matplotlib no se instala con `pip` system-wide en macOS por PEP 668):
   ```bash
   python3 -m venv temp/venv-claseXX
   source temp/venv-claseXX/bin/activate
   pip install --quiet matplotlib numpy
   ```

2. **Un script con todas las figuras**, una función por imagen, paleta global como constantes (`BRAND_DARK`, `ACCENT_BLUE`, etc.) coincidente con el CSS de la presentación. Usar `mpl.rcParams.update({...})` arriba para fijar fuente (`Helvetica Neue` en Mac), spines off, line widths, dpi 160.
   ```python
   def save(fig, name):
       fig.savefig(f'public/classroom/{slug}/slides/img/clase-XX-{name}.png')
       plt.close(fig)
   ```

3. **Regenerar todo o sólo una**:
   ```bash
   python temp/gen_claseXX_figs.py            # todas
   python -c "import sys; sys.path.insert(0,'temp'); from gen_claseXX_figs import fig_21_good; fig_21_good()"
   ```

4. **Verificar visualmente** con `Read` cada PNG (matplotlib silencia warnings; un eje desplazado o un mínimo en el lugar equivocado solo se ve abriendo la imagen).

5. **Reemplazar SVGs inline por `<img>`** con un script Python (`temp/replace_svgs.py`): mapeo `aria-label` → png. Para grids con muchos `<svg aria-hidden="true">` (mini-iconos sin label) reemplazar por contexto del `<span class="name">` siguiente.

### Tamaño y dpi
- 160 dpi con `figsize=(7.5, 5)` ≈ 1200×800 px — se ve nítido en pantallas retina y pesa <100 KB.
- Para mini-iconos (12 visuales del slide 14): `figsize=(4, 3)` con dpi 160 → ~640×480, <30 KB cada uno.

### Paleta consistente
```python
BRAND_DARK = '#1A1A1A';    BRAND_YELLOW = '#FFBF00';   BRAND_YELLOW_DARK = '#E6AC00'
BRAND_GRAY = '#555555';    GRAY_SOFT = '#9CA3AF';      GRAY_LIGHT = '#D1D5DB'
ACCENT_BLUE = '#0F62FE';   ACCENT_RED = '#DC2626';     ACCENT_GREEN = '#059669'
```

### Imágenes "deliberadamente malas" (antes / después)
Para mostrar mal diseño es legítimo activar todo lo que decimos no usar: `axes.spines.top/right=True` con `linewidth=2.5`, fondo gris, marca de agua con `ax.text(0.5, 0.5, 'BORRADOR', alpha=0.18, rotation=-15)`, leyenda con sombra (`fancybox=True`, manualmente con `Rectangle`), 8 series listadas pero solo 3 graficadas, decimales artificiosos (`1234.5678`), etiquetas X rotadas −45°, `set_axis_off()` desactivado y un borde grueso. La función vale como ejemplo *contraste*, no como descuido.

### Bug crítico: `$...$` dentro de `<text>` SVG

Cuando se reemplaza un `<svg>` por `<img>`, los textos KaTeX que estaban dentro de `<text>` SVG **NUNCA fueron renderizados** — `renderMathInElement` de KaTeX **no procesa contenido dentro de `<svg>`**. Si el render visual mostraba `R = 1{,}4` en vez de la fracción, era texto crudo, no LaTeX.

**Reemplazos válidos en SVG `<text>`:**
- `$R = 1{,}4$` → `R = 1,4`
- `$R / R_{\min}$` → `R / R<tspan font-size="7" baseline-shift="-25%">mín</tspan>`
- `$\Delta T_{\min}$` → `ΔT<tspan font-size="5" baseline-shift="-25%">mín</tspan>`
- `$R^2 = 0{,}97$` → `R² = 0,97` (Unicode superíndice)
- `$Q = 800$` → `Q = 800`

KaTeX **sí funciona** dentro de los matplotlib titles si están entre `$...$` con notación TeX nativa (`r'$\Delta T_{\min}$'`).

## Slides interactivos: stepper / toggle (`flip-group`)

Patrón probado en clase-18, slides 19 y 23. Genérico para alternar entre N variantes de una imagen sin colapsar el slide a un grid apretado.

### HTML
```html
<div class="flip-group" data-flip-group>
  <div class="flip-tabs" role="tablist" aria-label="Etiqueta del grupo">
    <button type="button" role="tab" class="is-active" data-target="A" aria-selected="true">Opción A</button>
    <button type="button" role="tab" data-target="B" aria-selected="false">Opción B</button>
  </div>
  <div class="flip-panels">
    <div class="flip-panel is-active" data-panel="A" role="tabpanel">
      <img src="img/clase-XX-A.png" alt="..." />
      <div class="flip-caption"><h3>Título</h3><p>Descripción.</p></div>
    </div>
    <div class="flip-panel" data-panel="B" role="tabpanel" hidden>
      <img src="img/clase-XX-B.png" alt="..." />
      <div class="flip-caption"><h3>...</h3><p>...</p></div>
    </div>
  </div>
</div>
```

### Variante toggle binario (Razón baja / Razón alta)
Agregar `flip-group--toggle` al contenedor y `flip-tabs--toggle` a la barra de tabs. Las pills usan colores de feedback semántico:
- `class="is-bad"` → activa en rojo (`--accent-red`)
- `class="is-good"` → activa en verde (`--accent-green`)

### CSS (resumen)
- `.flip-tabs button` — pills redondas, fondo blanco / activo `--brand-dark` blanco.
- `.flip-tabs--toggle` — `inline-flex` centrado, fondo `--muted`, padding 0.3rem.
- `.flip-panels` y `.flip-panel.is-active` con `display: grid; grid-template-columns: 7fr 5fr` (imagen + caption); colapsa a 1 columna en `<720px`.
- Animación `flipFade` 0.32s.
- Bloque completo en `clase-18-comunicacion-visual.html` (sección `Flip-group`).

### JS
```js
document.querySelectorAll('[data-flip-group]').forEach(group => {
  const tabs = group.querySelectorAll('.flip-tabs button');
  const panels = group.querySelectorAll('.flip-panel');
  ...
  tabs.forEach(b => {
    ...
    b.addEventListener('keydown', e => {
      if (e.key === 'ArrowRight' || e.key === 'ArrowLeft') {
        e.preventDefault();
        const arr = Array.from(tabs); const i = arr.indexOf(b);
        const next = e.key === 'ArrowRight' ? (i+1) % arr.length : (i-1+arr.length) % arr.length;
        arr[next].focus(); activate(arr[next].getAttribute('data-target'));
      }
    });
  });
});
...
Soporta navegación con `←` `→` entre tabs cuando el grupo tiene foco.

## Cards clickeables (`vis-cell.interactive`)

Para grids de cards donde cada celda revela un detalle al click sin abandonar el slide. Probado en clase-18 slides 14 y 20.

```html
<div class="vis-cell interactive" tabindex="0" role="button" aria-label="Detalle: Mapa de calor">
  <img src="img/clase-XX-heatmap.png" alt="Mapa de calor" />
  <span class="name">Mapa de calor</span>
  <span class="hint">Para análisis de sensibilidad de dos variables.</span>
</div>
```

CSS: `.hint` arranca con `max-height: 0; opacity: 0`. Al `:hover` o `.is-open` pasa a `max-height: 6rem; opacity: 1`. Indicador `+`/`−` en esquina superior derecha que rota al alternar. JS toggle con click + Enter/Espacio (idéntico al patrón `interactive-card` ya documentado).

## Otros patrones aprendidos

### Pills rectangulares con texto largo (no fijar `width`)
`.cascade-step .num` y similares deben usar `min-width: 1.8em; padding: 0 0.6em; white-space: nowrap` en lugar de `width: 1.8em; height: 1.8em`. Si el contenido es solo `1`, queda redondo (`min-width` se respeta como mínimo); si es `3 min`, se expande horizontalmente. Sin esto, los textos largos se solapan con el resto del step.

### Validación HTML void-aware con SVG children
El parser básico de `html.parser` no conoce los elementos void de SVG (`<circle>`, `<line>`, `<rect>`, `<path>`, `<polyline>`, `<polygon>`, `<ellipse>`, `<stop>`, `<use>`, `<feGaussianBlur>`). Sin agregarlos al set `VOID`, cada `<circle ... />` cuenta como abierto y el parser reporta cientos de "unclosed" falsos. Usar el snippet ampliado:
```python
VOID = {'area','base','br','col','embed','hr','img','input','link','meta','param','source','track','wbr',
        'circle','line','rect','path','polygon','polyline','ellipse','stop','use','feGaussianBlur'}
```
