...
export const PublicationSection = ({ publication, index, citationCount, isLoadingCitations }: PublicationSectionProps) => {
      ...
      >
        <div className="bg-white rounded-lg shadow-xl border border-yellow-400/40 p-8 md:p-12">
            <div className="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
                ...
                {(publication.researchAreas && publication.researchAreas.length > 0) || publication.status ? (
                ...
                <MetaItem label="Authors">
                  {publication.authors.map((author, i) => (
                    <React.Fragment key={i}>
                      {author === 'Luis H. Reyes' ? <strong className="font-bold text-slate-900">{author}</strong> : author}
                      {i < publication.authors.length - 1 ? ', ' : ''}
                    </React.Fragment>
                  ))}
                </MetaItem>
                <MetaItem label="Published In" value={`${publication.publicationVenue}, ${publication.publicationDate}`} />
                <MetaItem label={publication.type === 'Patent' ? 'Patent Number' : 'DOI'}>
                    <a
                        href={
                            publication.url
                            ? publication.url
                            : publication.type === 'Patent'
                            ? `https://patents.google.com/patent/${publication.doi}`
            ...
            <div className="mt-8">
                ...
                <AnimatePresence initial={false}>
                    {isExpanded && (
                        ...
                        >
                            <motion.div {...{variants: summaryVariants}} className="mt-6 space-y-6 pt-4 border-t border-slate-200">
                                {publication.laymanSummary.map((item, i) => (
            ...
            <div className="mt-8 pt-4 border-t border-slate-200">
                <p className="text-sm text-slate-500 font-medium">Keywords</p>
