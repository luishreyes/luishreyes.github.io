import type { GraduatedStudent } from '../../../../types';

const student: GraduatedStudent = {
    name: 'Julián Fernando Becerra-Encinales',
    degree: 'Doctor',
    program: { en: 'Doctor in Technological Innovation Management', es: 'Doctor en Gestión de la Innovación Tecnológica' },
    graduationYear: 2025,
    startedYear: 2021,
    currentPosition: { en: 'Director of Technological Extension at Cenipalma', es: 'Director de Extensión Tecnológica en Cenipalma' },
    linkedinUrl: 'https://www.linkedin.com/in/julian-fernando-becerra-encinales-4129a9326/',
    thesisTitle: 'Systemic Management Model for Technological Extension in Oil Palm Cultivation in Colombia: An Integrative and Evidence-Based Approach',
    laymanSummary: [
        {
            question: { en: 'What was the problem?', es: '¿Cuál era el problema?' },
            answer: { en: 'Technological extension in developing countries, especially for complex crops like oil palm, is often inefficient and doesn\'t adapt to local needs, limiting sustainability and productivity.', es: 'La extensión tecnológica en países en desarrollo, especialmente para cultivos complejos como la palma de aceite, suele ser ineficiente y no se adapta a las necesidades locales, lo que limita la sostenibilidad y la productividad.' }
        },
        {
            question: { en: 'What was the approach?', es: '¿Cuál fue el enfoque?' },
            answer: { en: 'This research used a mixed-methods approach, combining a literature review, cluster analysis of sustainability data from 3,808 Colombian oil palm producers, and a system dynamics model to create a comprehensive management framework.', es: 'Esta investigación empleó un enfoque de métodos mixtos, combinando una revisión de literatura, un análisis de conglomerados de datos de sostenibilidad de 3.808 productores colombianos de palma de aceite y un modelo de dinámica de sistemas para crear un marco de gestión integral.' }
        },
        {
            question: { en: 'What were the findings?', es: '¿Cuáles fueron los hallazgos?' },
            answer: { en: 'The study identified key barriers to technology adoption and segmented producers into distinct typologies based on their sustainability performance. It developed a practical, systemic model and an operational protocol for Cenipalma (the leading research institute) to implement more targeted, effective, and adaptive extension strategies, moving beyond one-size-fits-all approaches.', es: 'El estudio identificó barreras clave para la adopción de tecnología y segmentó a los productores en tipologías diferenciadas según su desempeño en sostenibilidad. Desarrolló un modelo sistémico y práctico y un protocolo operativo para que Cenipalma (el principal instituto de investigación) implemente estrategias de extensión más dirigidas, eficaces y adaptativas, superando los enfoques de talla única.' }
        }
    ],
    imageUrl: 'https://ourwyskhfdesnmnhlxof.supabase.co/storage/v1/object/public/Photos/Julian%20Fernando%20Becerra-Encinales.JPG',
};

export default student;
